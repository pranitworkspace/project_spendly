...
`app.py` marks its stub routes with a literal string naming the step that implements them (`"Delete expense — coming in Step 9"`), so the current state is always readable from the file itself. When a step lands, its route moves **above** the `# Placeholder routes` banner under its own `# <Feature> (Step N)` banner, so that section only ever holds what is still unbuilt.

Steps 1-8 are done: Step 1 (`database/db.py`), Step 2 (registration), Step 3 (login, session, logout), Step 4 (profile page UI), Step 5 (profile page wired to the database), Step 6 (the `/profile` date filter), Step 7 (add expense) and Step 8 (edit expense). `/profile` renders its four sections (user card, summary stats, transaction table, category breakdown) from **live `users`/`expenses` queries** via `database/db.py` — `templates/profile.html` was not changed to get there. Only Step 9 (delete expense) remains. As of Step 4 `/profile` is the post-login home: a successful login, and a signed-in visitor hitting `/login` or `/register`, all redirect to `url_for("profile")` (Step 3 parked these on the landing page only because no signed-in page existed yet). The signed-in navbar name links to `/profile`. Logout still redirects to the landing page.
...
- `_build_transactions()` carries a raw `id` through to the template alongside the pre-formatted display strings; it is what each row's action link feeds to `url_for('edit_expense', id=...)`. Row actions live in a trailing `.profile-tx-actions` cell — Step 9's Delete link belongs in that same cell.
